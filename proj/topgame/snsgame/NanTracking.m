...
static NSString *NAN_FB_APP_ID = nil;

static BOOL  NAN_ENABLED = NO;
static NSString *NAN_UID = nil;

+(void) initSettings
{
    if(NAN_ENABLED) {
        [NanTracking trackNanigansEvent:NAN_UID type:@"visit" name:@"dau"];
        return;
    }
    NSString *idfa = [SystemUtils getIDFA];
    if(idfa!=nil && [idfa length]>10) {
        NSString *fbID = [SystemUtils getSystemInfo:@"kFacebookAppID"];
        if(fbID!=nil && [fbID length]>3) {
            NAN_FB_APP_ID = [fbID retain];
            NAN_UID = [idfa retain];
            [NanTracking trackNanigansEvent:idfa type:@"install" name:@"main"];
            NAN_ENABLED = YES;
            [NanTracking trackNanigansEvent:NAN_UID type:@"visit" name:@"dau"];
        }
    }
...
{
    [NAN_FB_APP_ID release]; [NAN_UID release];
    NAN_UID = nil; NAN_FB_APP_ID  = nil; NAN_ENABLED = NO;
}

+ (void) setFbAppId:(NSString *) fbAppId {
    NAN_FB_APP_ID = [fbAppId retain];
}
...
{
    [NanTracking trackNanigansEvent:NAN_UID type:@"purchase" name:@"main" extraParams:@{@"value":price,@"sku":itemID, @"qty":@"1"}];
}
