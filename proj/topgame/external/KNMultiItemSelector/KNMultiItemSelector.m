...
-(void)loadView {
  ...
  //  self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Select All" style:UIBarButtonItemStyleDone target:self action:@selector(didSelectAll)];
    bSelectAllStatus = NO;
}
...
-(void)didSelectAll {
    ...
    if(bSelectAllStatus) {
        ...
        bSelectAllStatus = YES;
    }
     */
    if ([delegate respondsToSelector:@selector(selector:didFinishSelectionWithItems:)]) {
        [delegate selector:self didFinishSelectionWithItems:items];
    }
}
