...
#if (MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_5) && !TMBGTM_IPHONE_SDK
// General use would be to call this through TMBGTMCFAutorelease
...
#define TMBGTMNSMakeCollectable(cf) ((id)NSMakeCollectable(cf))

// TMBGTMNSMakeUncollectable is for global maps, etc. that we don't
// want released ever. You should still retain these in non-gc code.
TMBGTM_INLINE void TMBGTMNSMakeUncollectable(id object) {
  [[NSGarbageCollector defaultCollector] disableCollectorForPointer:object];
}

// Hopefully no code really needs this, but TMBGTMIsGarbageCollectionEnabled is
// a common way to check at runtime if GC is on.
// There are some places where GC doesn't work w/ things w/in Apple's
// frameworks, so this is here so TMBGTM unittests and detect it, and not run
// individual tests to work around bugs in Apple's frameworks.
TMBGTM_INLINE BOOL TMBGTMIsGarbageCollectionEnabled(void) {
  return ([NSGarbageCollector defaultCollector] != nil);
}

#else

#define TMBGTMNSMakeCollectable(cf) ((id)(cf))
...
// TMBGTMCFAutorelease makes a CF object collectable in GC mode, or adds it 
...
#define TMBGTMCFAutorelease(cf) ([TMBGTMNSMakeCollectable(cf) autorelease])
